...
// 1. Importamos a tela do Dashboard diretamente do outro arquivo!
...
function LoginScreen() {
  ...
  // 2. Criamos uma "chave" que diz se estamos logados ou não
  const [isAuthenticated, setIsAuthenticated] = useState(false);

  const handleLogin = () => {
    // 3. Ao clicar, viramos a chave (sem acionar o roteador)
    setIsAuthenticated(true);
  };

  // 4. A mágica: se a chave estiver virada, a tela muda instantaneamente!
  if (isAuthenticated) {
        ...
        <div className="p-6 sm:p-8 overflow-y-auto">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-col sm:flex-row bg-slate-100 p-1 rounded-lg gap-1 sm:gap-0">
              <button onClick={() => setRole('atendente')} className={`flex-1 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-md flex items-center justify-center gap-2 transition-all ${role === 'atendente' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <User className="w-4 h-4" /> Atendente
              </button>
              <button onClick={() => setRole('policial')} className={`flex-1 py-2 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-md flex items-center justify-center gap-2 transition-all ${role === 'policial' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
                <Shield className="w-4 h-4" /> Agente / Policial
              </button>
            </div>
          </div>
          ...
          <form className="space-y-4 sm:space-y-5">
            <div>
              <label className="block text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">ID do Operador / Email</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><User className="h-4 w-4 sm:h-5 sm:w-5 text-slate-400" /></div>
                <input type="text" className="block w-full pl-9 sm:pl-10 pr-3 py-2 sm:py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-blue-600 bg-slate-50 text-slate-900 text-xs sm:text-sm" placeholder="Insira suas credenciais" />
              </div>
            </div>

            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="block text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider">Chave de Acesso</label>
                <a href="#" className="text-[10px] sm:text-xs font-bold text-blue-600 hover:text-blue-800">RECUPERAR ACESSO</a>
              </div>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><Key className="h-4 w-4 sm:h-5 sm:w-5 text-slate-400" /></div>
                <input type="password" className="block w-full pl-9 sm:pl-10 pr-3 py-2 sm:py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-blue-600 bg-slate-50 text-slate-900 text-xs sm:text-sm" placeholder="••••••••" />
              </div>
            </div>
            ...
            {/* Vinculamos a função handleLogin ao clique */}
            <button
              type="button"
              onClick={handleLogin}
